...
class DAO():
    ...
    def getCorsiPD(pd):
        ...
        res = []
        for row in cursor:
            res.append(Corso(**row)) #se i nomi delle colonne del database sono uguali ai nomi degli oggetti del dao possiamo passare direttamente i valori facendo **row
    ...
    def getCorsiPDwIscritti(pd):
        ...
        res = []
        for row in cursor:
            res.append((Corso(codins=row["codins"], crediti=row["crediti"], nome=row["nome"], pd=row["pd"]), row["n"]))
