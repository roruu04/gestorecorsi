...
class Controller:
    ...
    def handlePrintCorsiPD(self, e):
        ...
        if pd is None :
            (self._view.create_alert("Attenzione, selezionare periodo didattico."))
        ...
        self._view.txt_result.controls.append(
            ft.Text(f"Di seguito i corsi dei {pd} periodo didattico."))
        for c in corsiPD:
            self._view.txt_result.controls.append(
                ft.Text(c)
            )
    ...
    def handlePrintIscrittiCodins(self, e):
        ...
        studenti = self._model.getStudentiCorsi(self._ddCodinsValue.codins)
        if not len(studenti):
            self._view.txt_result.controls.append(
    ...
    def handlePrintCDSCodins(self, e):
        ...
        if self._ddCodinsValue is None:
            ...
            self._view.update_page()
            return
        cds = self._model.getCDSodCorso(self._ddCodinsValue.codins)
        if not len(cds):
            self._view.txt_result.controls.append(
    ...
    def fillddCodins(self): #controller li chiede al model per inserirli nella pagina
